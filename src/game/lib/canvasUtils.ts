    ...
    board.forEach((row, rowIndex) => {
        row.forEach((tile, colIndex) => {
            if (tile !== EMPTY && images[tile - 1]) {
                ...
                // 네모 모양으로 클리핑
                ctx.beginPath();
                ctx.rect(actualCenter.x - tileRadius, actualCenter.y - tileRadius, tileSize, tileSize);
                ctx.clip();
...
export function drawPath(
    ...
    if (start.x < 0 || start.x >= ROWS || start.y < 0 || start.y >= COLS) {
        ...
        startX = Math.max(PADDING - GRID_SIZE/2, Math.min(start.y * GRID_SIZE + GRID_SIZE / 2 + PADDING, COLS * GRID_SIZE + GRID_SIZE/2 + PADDING));
        startY = Math.max(PADDING - GRID_SIZE/2, Math.min(start.x * GRID_SIZE + GRID_SIZE / 2 + PADDING, ROWS * GRID_SIZE + GRID_SIZE/2 + PADDING));
    } else {
    ...
    for (let i = 1; i < path.length; i++) {
        ...
        if (point.x < 0 || point.x >= ROWS || point.y < 0 || point.y >= COLS) {
            // 외부 경로인 경우 보드 경계에 맞춰 조정
            x = Math.max(PADDING - GRID_SIZE/2, Math.min(point.y * GRID_SIZE + GRID_SIZE / 2 + PADDING, COLS * GRID_SIZE + GRID_SIZE/2 + PADDING));
            y = Math.max(PADDING - GRID_SIZE/2, Math.min(point.x * GRID_SIZE + GRID_SIZE / 2 + PADDING, ROWS * GRID_SIZE + GRID_SIZE/2 + PADDING));
        } else {
